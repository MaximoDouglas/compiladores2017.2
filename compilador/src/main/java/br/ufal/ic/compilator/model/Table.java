...
public class Table {
	...
	public void populateTable() throws FileNotFoundException {
		...
		table.put(new Pair("CodAux","tipo_void'"),"CodAux = Funcao");
		...
		table.put(new Pair("CodAux","tipo_bool'"),"CodAux = Funcao");
		...
		table.put(new Pair("CodAux","tipo_int'"),"CodAux = Funcao");
		...
		table.put(new Pair("CodAux","tipo_float'"),"CodAux = Funcao");
		...
		table.put(new Pair("CodAux","tipo_char'"),"CodAux = Funcao");
		...
		table.put(new Pair("CodAux","‘retorno’"),"CodAux = Funcao");
	...
	public void getDerivation(String nTerm, String term) {
		Pair p = new Pair(nTerm, term);
		String production = table.get(p);
		String aux[] = null;

		while (table.containsKey(p)) {
			aux = production.split("=");

			for (int i = 1; i < aux.length; i++) {
				stack.push(aux[i].trim());
			}

			p = new Pair(stack.peek(), term);
			production = table.get(p);
			System.out.println("C" + production);
			System.out.println("Aqui: " + stack.peek() + "," + term);

			if (term.equals(stack.peek())) {
				stack.pop();
				break;
			}
		}
